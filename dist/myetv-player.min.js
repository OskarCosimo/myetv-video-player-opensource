...
reinitializeSubtitles() {
    ...
    if (defaultTrack !== -1 && this.options.subtitlesEnabled === true) { 
        setTimeout(() => {
...
getAutoQualityBasedOnConnection() {
    ...
    if (connection) {
        const physicalType = connection.type;
        ...
        if (physicalType === 'wifi') {
            const quality = findQualityByMinHeight(1440) || findHighestQuality();
        ...
        if (physicalType === 'cellular') {
            ...
            if (downlinkSpeed >= 20 && rtt < 40) {
                const quality = findQualityByMinHeight(1080);
...
parseCustomSRT(srtText) {
    var subtitles = [];
    var normalizedText = srtText.replace(/\r /g, ' ').replace(/\r/g, ' ');
...
initializeWatermark() {
    ...
    watermark.classList.add(`watermark-${position}`);
